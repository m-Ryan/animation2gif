...
const haveDirectionProperties = lodash.exports.flatMap([
  "padding",
  "margin",
  "border"
...
function cloneNodeWithInlineStyle(node) {
...
var GIF = gif.exports;
const generateGif = (options) => {
  const { perFrameTime = 1e3 / 24, gif: gifOption, images } = options;
  return new Promise((resolve) => {
    const gif2 = new GIF(__spreadValues({
      workers: 2,
      quality: 1,
...
function animationToGIF(list, options) {
  return __async(this, null, function* () {
    ...
    const imageList = yield Promise.all(framesImages.map((svg) => {
      return new Promise((resolve) => {
        const img = new Image();
...
export { animationToGIF, dom2Svg, generateGif };
